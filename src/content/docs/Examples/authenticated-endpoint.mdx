...
target_link_libraries(authenticated-api PRIVATE Zuno::Zuno)`}
/>
...
int main() {
  ...
  app.get("/private", authMiddleware, [](Request& req, Response& res) {
...
Middleware authMiddleware = [](Request& req, Response& res, Next next) {
...
- Add [Rate Limiting](/extensions/rate-limiting) to protect login endpoints
- Use [TLS](/guide/tls-security) to encrypt credentials
- Return [JSON error responses](/guide/error-handling) for consistency
